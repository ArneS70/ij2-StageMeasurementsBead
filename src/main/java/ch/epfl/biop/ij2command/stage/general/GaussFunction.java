...
public class GaussFunction {
	 ...
	 * @param y	target data
	 */
	public GaussFunction(List<Double> x, List<Double> y) {
		this.x=x;
		this.y=y;
	}
		...
		return new MultivariateVectorFunction() {
			...
			public double[] value(double[] variables)
		        ...
		        for (int i = 0; i < values.length; ++i) {
		        	values[i] = variables[0]+variables[1]*Math.pow(Math.E, -1*(x.get(i)*x.get(i))/variables[2]);
    	...
    	return new MultivariateMatrixFunction() {
		    ...
		    private double[][] jacobian(double[] variables) {
		        ...
		        for (int i = 0; i < jacobian.length; ++i) {
		            jacobian[i][0] = 1.0;
		            jacobian[i][1] = Math.pow(Math.E, -1*(x.get(i)*x.get(i))/variables[2]);
		            jacobian[i][2] = variables[1]*Math.pow(Math.E, -1*(x.get(i)*x.get(i))/variables[2])*x.get(i)*x.get(i)/(variables[2]*variables[2]);
		        }
		        return jacobian;
		    }
